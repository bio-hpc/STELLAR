...
from .tools import *
from fix_gro_zero_coords import fix_gro_zero_coords
...
def create_target_queries_gro(lst_class_queries,  prefix_out, cfg):
    ...
    if len(lst_class_queries) == 0:
        ...
        out_gro = prefix_out+cfg.prefix_sumulation+".gro"
        shutil.move(cfg.target_file_gro, out_gro)
        fixed = fix_gro_zero_coords(out_gro)
        if fixed:
            print("Repaired {} zero-coordinate atom(s) in {}".format(fixed, out_gro))
        return
    else:
        sum_atoms_queries = 0
        ...
        out_gro = prefix_out + cfg.prefix_sumulation + ".gro"
        write_file(complex_gro, out_gro)
        fixed = fix_gro_zero_coords(out_gro)
        if fixed:
            print("Repaired {} zero-coordinate atom(s) in {}".format(fixed, out_gro))
...
def create_topology_target(cfg):
    ...
    for line in execute_cmd(cmd):
        ...
        if line.startswith("Total charge in system"):
            charge = line.split(" ")[4]
    os.chdir(cfg.path)
    fixed = fix_gro_zero_coords(cfg.target_file_gro)
    if fixed:
        print("Repaired {} zero-coordinate atom(s) in {}".format(fixed, cfg.target_file_gro))
