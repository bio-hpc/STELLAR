...
execute_script()
{
    ...
    #
    #   Analizar resultados (optional; MD outputs already written above)
    #
    if command -v singularity >/dev/null 2>&1; then
        echo "singularity exec --bind ${PWD} ${path_singularity}ASGARD.simg ${python_run}  ${path_analize_results}Simulation_gromacs/analyze_trajectory/get_results.py ${out_molec}_complex ${mode_gr} $prefix_gromacs"
        singularity exec --bind ${PWD} ${path_singularity}ASGARD.simg ${python_run}  ${path_analize_results}Simulation_gromacs/analyze_trajectory/get_results.py ${out_molec}_complex ${mode_gr} $prefix_gromacs || echo "WARNING: get_results.py failed (non-fatal)"
    else
        echo "WARNING: singularity not in PATH; skipping get_results.py"
    fi
