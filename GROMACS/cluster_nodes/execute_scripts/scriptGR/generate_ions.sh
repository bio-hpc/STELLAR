...
{
    name_solv="CA"
	execute "${prefix_gromacs} grompp${mpi} -f ${out_grid}_tpr.mdp  -c ${out_molec}_complex_solv.gro -p ${out_molec}_complex.top -o ${out_aux}_ions.tpr -po ${out_aux}_grompp_IONS.mdp -maxwarn 3" #remplaza iones por atomos de agua tantos sean necesarios para estabilizar la carga en la target
	#numIons=`cat ${out_molec}_topol.top |grep "; residue" |awk '{suma+=$8} END {print suma}'` #numeeo de iones para nivelar la topologia
	# Ceil ion count so fractional net charge is neutralized (1.491 -> 2 Cl-)
	num_ions=$(awk -v c="$system_charge" 'BEGIN{
		c+=0
		if (c > 0) { printf "%d", int(c + 0.999) }
		else if (c < 0) { printf "%d", int(-c + 0.999) }
		else { print 0 }
	}')
