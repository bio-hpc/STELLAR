...
equilibration_nvt() #calentar la conformacion
{
	...
	#	Make the complex whole across PBC before NVT. Large receptors get split by the
	#	dodecahedron box (editconf -c centers but does not reassemble molecules), which
	#	makes grompp abort with "distance between excluded atoms larger than the cut-off".
	#	Reassemble each molecule using the minimization tpr.
	execute "echo System | ${prefix_gromacs} trjconv${mpi} -s ${out_molec}_complex_min.tpr -f ${out_molec}_complex_min.gro -pbc whole -o ${out_molec}_complex_min_whole.gro"
	execute "mv ${out_molec}_complex_min_whole.gro ${out_molec}_complex_min.gro"
	...
	#execute "${prefix_gromacs} mdrun${mpi} -deffnm ${out_molec}_complex_nvt ${gpu} ${thrads} -ntmpi 1 -g ${out_aux}_complex_mdrun_nvt.log"
  execute "${prefix_gromacs} mdrun${mpi} -deffnm ${out_molec}_complex_nvt ${gpu} -g ${out_aux}_complex_mdrun_nvt.log"
...
equilibration_npt() #se ejecuta 4 veces incrementando la posicion de restriccion
{
    ...
    execute "$prefix_gromacs mdrun${mpi} -deffnm ${out_molec}_complex_npt_${ini} ${gpu} -g ${out_aux}_comlex_npt_${ini}.log"
